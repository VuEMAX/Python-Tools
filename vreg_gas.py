...
def insert_data():
    ...
    for postcode_input in postcodes:
        try:
            ...
            URL = "https://www.vtest.be/Compare/Result?Environment=Hh&postcode=" + postcode_input + "&postcodeId=" + str(postcode_id) +"&vergelijkingstype=aardgas&type=hh&ElektriciteitVermogenZonnepanelen=&elektriciteitVerbruikBegin=&elektriciteitVerbruikEinde=&kentAardgasverbruik=false&aardgasVerbruikBegin=&aardgasVerbruikEinde=&aardgasVerwarming=true&gasWorkingHours=true&gasContractvoorkeur=vast"
            ...
            for row in rows:
                ...
                if (json_object["EnergyType"] == "G"):
                    ...
                    price = float(json_object["Price"]) / 34890
                    data['price'] = round(price, 4)
                    data_list.append(data)
                
            # Create Pandas Dataframe
            df_bs = pd.DataFrame(data_list,columns=['postal_code','supplier', 'supplier_url', 'product', 'type', 'duration', 'conditions','price'])
            df_bs.sort_values(by=['price'], inplace=True, ascending=True)

            df_bs.loc[(df_bs['type'] == 'Vast') & (df_bs['duration'] == '1')].head(10).to_sql('gas_offer', con=engine, if_exists='append', index=False)
